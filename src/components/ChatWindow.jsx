...
const ChatWindow = () => {
  ...
  // Track which message indices have already finished animating
  ...
  const handleAnimationComplete = (index) => {
    ...
    // Ensure we scroll to bottom as text expands and resources appear
    setTimeout(scrollToBottom, 100);
  };

  // Helper to detect if a message warrants an App CTA
  const shouldShowAppCTA = (content) => {
    const keywords = ['GoMate App', 'personalized plan', 'tailored guidance', 'comprehensive plan'];
    return keywords.some(keyword => content.includes(keyword));
  };

  // Static markdown styles for already-rendered messages
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 w-[calc(100vw-32px)] sm:w-[400px] h-[550px] max-h-[calc(100vh-100px)] bg-white rounded-2xl shadow-2xl border border-stone-200 z-[60] flex flex-col overflow-hidden font-sans"
        >
          {/* Header */}
          <div className="bg-sage-600 p-4 flex items-center justify-between shrink-0 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-white text-base tracking-wide">GoMate Assistant</h3>
                <p className="text-sage-100 text-xs font-medium">Always here to help</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto bg-stone-50 p-4 space-y-4">
            {messages.map((msg, idx) => {
              ...
              // Check if this is the latest AI message that should be typed out
              // Only type out if it hasn't been animated yet AND it's the last message
              ...
              return (
                <div key={idx} className="flex flex-col gap-2">
                  ...
                  >
                    <div 
                      className={`max-w-[85%] p-3.5 rounded-2xl text-sm shadow-sm transition-all duration-200 ${
                        isUser 
                      ...
                      {!isUser && !msg.isError ? (
                        shouldAnimateTypewriter ? (
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            {/* Render CTA for static/historical messages */}
                            {shouldShowAppCTA(msg.content) && (
                              <AppRecommendationCTA />
                            )}

                            {/* Render Resources for static/historical messages */}
